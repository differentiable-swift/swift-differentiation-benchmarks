...
struct Solution: Differentiable {
    ...
    static func optimization(
        ...
        for opt in 1 ... iterations {
            var (_, deltaInitialWaterLevel) = valueWithGradient(at: initialWaterLevel) {
                initialWaterLevel -> Float in
                var solution = Solution(waterLevel: initialWaterLevel)
                for step in 0 ..< duration {
                    solution = solution.evolve()
                }
                let loss = solution.currentWaterLevel.meanSquaredError(to: target)
                return loss
            }
